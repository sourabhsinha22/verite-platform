...
export async function sendEmail({ to, subject, html, fromName }: { to: string; subject: string; html: string; fromName?: string }) {
  if (!RESEND_API_KEY) {
  ...
  const resp = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: `${senderName} <${FROM_EMAIL}>`, to, subject, html }),
  })
